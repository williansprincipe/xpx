...
rvEnum Plot2D::script_file_path_parse(std::string script_file_path_string)
...
rvEnum Plot2D::data_file_path_parse(std::string data_file_path_string)
...
rvEnum Plot2D::writeScriptHeader(std::ofstream& ofs)
...
rvEnum Plot2D::writeScript(std::ofstream& ofs)
...
rvEnum Plot2D::writeData(std::ofstream& ofs)
...
rvEnum Plot2D::writeEmbeddedData(std::ofstream& ofs)
{ auto& m = sequences().values_matrix();
  auto& p = sequences_to_plot_;
  ofs << std::fixed;
  for (vsz_t j = 0; j < p.size(); ++j)
...
rvEnum Plot2D::writePlotAs(const std::string& file_name)
